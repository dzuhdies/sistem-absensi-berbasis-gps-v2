...
<head>
    <meta charset="UTF-8">
    <title>Export Rekap Absensi</title>
    <style>
        /* --- TEMA DAN LAYOUT --- */
        body {
...
<body>
    ...
    <div class="container">
        <div class="report-header">
            ...
            <p><strong>Gedung:</strong> {{ $gedung->nama ?? 'Semua Gedung' }}</p>
            <p><strong>Laporan Dibuat:</strong> {{ now()->translatedFormat('d F Y, H:i') }}</p>
        </div>

        {{-- Logika untuk memilih tabel yang benar --}}
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        <a href="{{ $absen->foto_masuk_url }}" target="_blank"><img src="{{ $absen->foto_masuk_url }}" alt="Foto Masuk"></a>
                        @else - @endif
                    </td>
                    <td class="text-center">
                        @if ($absen->lokasi_masuk_lat && !$absen->izin)
                        <a href="https://www.google.com/maps?q={{ $absen->lokasi_masuk_lat }},{{ $absen->lokasi_masuk_long }}" target="_blank">Lihat Peta</a>
                        @else - @endif
                    </td>
                    <td class="text-center">
                        @if ($absen->foto_keluar_url && !$absen->izin)
                        <a href="{{ $absen->foto_keluar_url }}" target="_blank"><img src="{{ $absen->foto_keluar_url }}" alt="Foto Keluar"></a>
                        @else - @endif
                    </td>
                </tr>
                @empty
                ...
                @empty
                <tr>
                    <td colspan="10" class="text-center" style="padding: 20px;">Tidak ada siswa yang cocok dengan filter.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        @else
        {{-- Tabel untuk mode harian --}}
        <table>
            <thead>
                <tr>
                    <th>Nama Siswa</th>
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="9" class="text-center" style="padding: 20px;">Tidak ada siswa yang terdaftar untuk ditampilkan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
